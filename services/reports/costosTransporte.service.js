  ...
  async generateHeader(doc, data) {
    ...
    switch (data.tipo) {
      ...
      case 'DI':
        ...
        doc.font('Helvetica-Bold');
        doc.fillColor('#444444');
        doc.fontSize(12).text('Fecha: 27/02/2022', 650, 30);
    ...
    switch (data.tipo) {
      case 'DE':
        ...
        for (var item = 0; item < detalles.length; item++) {
          ...
          if (data.dolar == 'true') {
            total_dolar += utils.parseFloatN(detalles[item]['movimientos.monto_dolar']);
            doc.y = ymin + i;
            doc.x = 520;
            doc.text(utils.formatNumber(detalles[item]['movimientos.monto_dolar']), {
              align: 'right',
              columns: 1,
              width: 40,
            });
      ...
      case 'RE':
        ...
        for (var item = 0; item < detalles.length; item++) {
          ...
          if (data.dolar == 'true') {
            total_dolar += utils.parseFloatN(detalles[item]['movimientos.total_dolar']);
            doc.y = ymin + i;
            doc.x = 520;
            doc.text(
        ...
        doc.y = ymin + i + 5;
        doc.x = 80;
        doc.text(utils.formatNumber(total_guias), {
          align: 'right',
          columns: 1,
          width: 50,
        });
        doc.y = ymin + i + 5;
        doc.x = 145;
        doc.text(utils.formatNumber(total_pzas), {
          align: 'right',
          columns: 1,
          width: 30,
        });
