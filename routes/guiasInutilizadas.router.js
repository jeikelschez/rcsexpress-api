...
router.get('/',
  ...
  async (req, res, next) => {
  try {
    const { page, limit, order_by, order_direction, agencia, tipo, nro_guia } = req.headers;
    const gInutilizadas = await service.find(page, limit, order_by, order_direction, agencia, tipo, nro_guia);
