...
router.get('/',
  ...
  async (req, res, next) => {
  try {
    const { page, limit, order_by, order_direction, filter, filter_value, estado, desc } = req.headers;
    const ciudades = await service.find(page, limit, order_by, order_direction, filter, filter_value, estado, desc);
    res.json(ciudades);
  } catch (error) {
    next(error);
  }
